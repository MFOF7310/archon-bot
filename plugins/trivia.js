...
run: async (client, message, args, db, serverSettings, usedCommand, lang) => {
        try {
            ...
            // PER-SERVER: Extract guildId for composite key isolation
            ...
            if (!userData) {
                // PER-SERVER: INSERT includes guild_id (composite PK)
                ...
                if (i.customId === 'trivia_category') {
                    ...
                    diffCollector.on('collect', async (j) => {
                        ...
                        if (j.customId === 'trivia_difficulty') {
                            ...
                            // PER-SERVER: All lookups and updates include guildId
                            const currentUserData = client.getUserData ? client.getUserData(userId, guildId) : userData;
                            if (client.queueUserUpdate) client.queueUserUpdate(userId, guildId, { ...currentUserData, credits: (currentUserData.credits || 0) - diff.bet, username: userName });
                            ...
                            // PER-SERVER: Final score update with guildId
                            const finalUserData = client.getUserData ? client.getUserData(userId, guildId) : userData;
                            if (finalUserData) {
                                const newXp = (finalUserData.xp || 0) + xpGain, newLevel = calculateLevel(newXp);
                                // PER-SERVER: queueUserUpdate and UPDATE both include guildId
                                if (client.queueUserUpdate) client.queueUserUpdate(userId, guildId, { ...finalUserData, credits: (finalUserData.credits || 0) + totalReward, xp: newXp, level: newLevel, games_played: (finalUserData.games_played || 0) + 1, games_won: (finalUserData.games_won || 0) + (correctAnswers >= questions.length / 2 ? 1 : 0), username: userName });
                                else db.prepare("UPDATE users SET credits = credits + ?, xp = xp + ?, level = ?, games_played = COALESCE(games_played, 0) + 1, games_won = COALESCE(games_won, 0) + ? WHERE id = ? AND guild_id = ?").run(totalReward, xpGain, newLevel, correctAnswers >= questions.length / 2 ? 1 : 0, userId, guildId);
                                
// ================= ASSIGN QUIZ MASTER ROLE =================
if (correctAnswers >= questions.length / 2) {
    if (message.guild) {
        try {
            ...
            const quizMasterRoleId = settings?.quizMasterRoleId || process.env.QUIZ_MASTER_ROLE_ID;
            if (quizMasterRoleId) {
                const member = await message.guild.members.fetch(userId).catch(() => null);
                if (member) {
                    const role = message.guild.roles.cache.get(quizMasterRoleId);
                    if (role && !member.roles.cache.has(quizMasterRoleId)) {
                        await member.roles.add(role, '🧠 Trivia champion').catch(() => {});
                    }
                }
            }
        } catch (e) {}
                            ...
                            // PER-SERVER: Display lookup includes guildId
                            const displayUserData = client.getUserData ? client.getUserData(userId, guildId) : finalUserData;
                            ...
                            finalCollector.on('collect', async (k) => {
                                ...
                                const freshMsg = {
                                    ...
                                    guild: k.guild,
                                    channel: k.channel,
                                    reply: async (opts) => k.followUp(opts),
                                ...
                                if (k.customId === 'trivia_again') {
                                    ...
                                    finalCollector.stop();
                                    const cmd = client.commands.get('trivia');
                                    if (cmd) await cmd.run(client, freshMsg, [], db, serverSettings, usedCommand);
                                }
                                    ...
                                    if (cmd) await cmd.run(client, freshMsg, ['menu'], db, serverSettings, usedCommand);
                                }
                            });
                        }
                    });
