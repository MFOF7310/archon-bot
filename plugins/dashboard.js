...
async function generateDashboardEmbed(client, message, db, showServerStats = true) {
    ...
    try {
        if (db) {
            // ================= PER-SERVER STATS (Composite Key Scoped) =================
            const scopeGuildId = guild?.id;

            if (scopeGuildId) {
                // Scoped to current guild — no cross-server data leakage
                dbUserCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE guild_id = ?").get(scopeGuildId)?.count || 0;
                totalXP = db.prepare("SELECT SUM(xp) as total FROM users WHERE guild_id = ?").get(scopeGuildId)?.total || 0;
                avgLevel = db.prepare("SELECT AVG(level) as avg FROM users WHERE guild_id = ?").get(scopeGuildId)?.avg || 0;

                serverDbStats.memberCount = dbUserCount;
                serverDbStats.serverXP = totalXP;
                // Use last_xp_gain (epoch ms) instead of non-existent last_seen
                serverDbStats.activeMembers = db.prepare(
                    "SELECT COUNT(*) as count FROM users WHERE guild_id = ? AND last_xp_gain > ?"
                ).get(scopeGuildId, Date.now() - (7 * 86400000))?.count || 0;

                serverDbStats.topMember = db.prepare(
                    "SELECT username, level FROM users WHERE guild_id = ? ORDER BY xp DESC LIMIT 1"
                ).get(scopeGuildId);
            } else {
                // In DMs — show global stats
    ...
    if (showServerStats && guild) {
        ...
        const serverAvgLevel = serverDbStats.memberCount > 0 
    ? db.prepare(`SELECT AVG(level) as avg FROM users WHERE guild_id = ?`).get(guild.id)?.avg?.toFixed(1) || '0.0'
    : '0.0';
