...
function createProgressBar(percent, length = 15) { const filled = Math.round((percent / 100) * length); const empty = length - filled; return '█'.repeat(Math.max(0, filled)) + '░'.repeat(Math.max(0, empty)); }

// ================= BILINGUAL TRANSLATIONS =================
const translations = {
    fr: {
        title: (name) => `📜 DOSSIER AGENT: ${name.toUpperCase()}`,
        node: 'Nœud', status: 'Statut', syncOk: '🟢 SYNC_OK', classification: 'Classification',
        syncTelemetry: '📊 TÉLÉMÉTRIE NEURALE', level: 'Niveau', rank: 'Classement', total: 'Total',
        xp: 'XP', messages: 'Messages', games: 'Parties', winRate: '% VICTOIRES', progress: '🚀 PROGRESSION',
        syncGap: 'XP requis', combat: '🎮 MATRICE DE COMBAT', primarySector: 'SECTEUR PRINCIPAL',
        combatMode: 'MODE COMBAT', rankTier: 'RANG/ÉCHELON', awaiting: 'EN_ATTENTE', setGame: 'Utilisez .setgame',
        noData: (name) => `❌ **Agent ${name}** n'a aucune donnée enregistrée.`,
        agentSince: 'Agent depuis', neuralEfficiency: 'Efficacité Neurale', dailyStreak: '🔥 Série Quotidienne',
        credits: '💰 Crédits', wealthTier: 'Niveau de Richesse',
        architectTitle: '🏛️ RECONNAISSANCE ARCHITECTE', architectValue: 'Le Créateur marche parmi nous. Le Système honore son Architecte.',
        digitalSovereignty: 'SOUVERAINETÉ NUMÉRIQUE', totalWinnings: 'Gains Totaux', winLoss: 'Victoires/Défaites'
    },
    en: {
        title: (name) => `📜 AGENT DOSSIER: ${name.toUpperCase()}`,
        node: 'Node', status: 'Status', syncOk: '🟢 SYNC_OK', classification: 'Classification',
        syncTelemetry: '📊 NEURAL TELEMETRY', level: 'Level', rank: 'Rank', total: 'Total',
        xp: 'XP', messages: 'Messages', games: 'Games', winRate: 'WIN %', progress: '🚀 PROGRESS',
        syncGap: 'XP needed', combat: '🎮 COMBAT MATRIX', primarySector: 'PRIMARY SECTOR',
        combatMode: 'COMBAT MODE', rankTier: 'RANK/TIER', awaiting: 'AWAITING', setGame: 'Use .setgame',
        noData: (name) => `❌ **Agent ${name}** has no recorded data.`,
        agentSince: 'Agent Since', neuralEfficiency: 'Neural Efficiency', dailyStreak: '🔥 Daily Streak',
        credits: '💰 Credits', wealthTier: 'Wealth Tier',
        architectTitle: '🏛️ ARCHITECT RECOGNITION', architectValue: 'The Creator walks among us. System honors its Architect.',
        digitalSovereignty: 'DIGITAL SOVEREIGNTY', totalWinnings: 'Total Winnings', winLoss: 'Wins/Losses'
    }
};

module.exports = {
    name: 'rank',
    // CRITICAL FIX: Added 'profile' alias (was missing!)
    ...
    category: 'PROFILE',
    usage: '.rank [@user] | .profile | .level',
    cooldown: 3000,
    examples: ['.rank', '.rank @user', '.profile', '/rank'],
    ...
    run: async (client, message, args, db, serverSettings, usedCommand, lang) => {
        try {
            ...
            // Resolve target: mention > author
            const target = message.mentions.users.first() || message.author;
            const userId = target.id;
            const userName = target.username;

            // ===== STEP 1: Try getUserData (cached or DB) =====
            let userData = null;
            try {
                if (client.getUserData) {
                    userData = client.getUserData(userId, guildId);
                }
            } catch (e) {
                console.log(`[RANK] getUserData error: ${e.message}`);
            }

            // ===== STEP 2: Fallback to direct DB query =====
            if (!userData && db) {
                try {
                    userData = db.prepare(
                        `SELECT id, xp, credits, streak_days, created_at, games_played, games_won, 
                         total_messages, total_winnings, gaming, level, username 
                         FROM users WHERE id = ? AND guild_id = ?`
                    ).get(userId, guildId);
                } catch (e) {
                    console.log(`[RANK] DB query error: ${e.message}`);
                }
            }

            // ===== STEP 3: Auto-create if missing (getOrCreateUser) =====
            if (!userData) {
                if (client.getOrCreateUser) {
                    try {
                        userData = client.getOrCreateUser(userId, guildId, userName);
                    } catch (e) {
                        console.log(`[RANK] getOrCreateUser error: ${e.message}`);
                    }
                }
            }
            ...
            if (!userData) {
                const errorEmbed = new EmbedBuilder()
                    .setColor('#ED4245')
                    .setAuthor({ name: t.title(target.username), iconURL: target.displayAvatarURL() })
                    .setDescription(t.noData(target.username))
                    .setFooter({ text: `${guildName} • ${t.digitalSovereignty} • v${version}`, iconURL: guildIcon })
                    .setTimestamp();
            ...
            // Rank calculation (fast indexed query)
                ...
                .addFields(
                    {
                        ...
                        value: `\`\`\`yaml\n${t.level}: ${level}\n${t.rank}: #${rank}/${totalUsers}\n${t.xp}: ${xp.toLocaleString()}\n${t.messages}: ${totalMessages.toLocaleString()}\n${t.games}: ${gamesPlayed.toLocaleString()} (${winRate}%)\`\`\``, inline: true
                    },
                    {
                        name: `💰 ${t.credits}`,
                        value: `\`\`\`yaml\n${t.credits}: ${credits.toLocaleString()} 🪙\n${t.wealthTier}: ${wealthTier.emoji} ${wealthTier.title[lang]}\n${t.totalWinnings}: ${totalWinnings.toLocaleString()} 🪙\`\`\``, inline: true
                    },
                    {
                        name: t.progress,
                        value: `\`\`\`\n${progressBar} ${percent.toFixed(1)}%\n└─ ${xpRemaining.toLocaleString()} XP ${t.syncGap}\`\`\``, inline: false
                    },
                    {
                        name: `🔥 ${t.dailyStreak}`, value: `\`${streakDays}\` ${lang === 'fr' ? 'jours' : 'days'}`, inline: true
                    },
                    {
                        name: `🧠 ${t.neuralEfficiency}`, value: `\`${neuralEfficiency}%\``, inline: true
                    },
                    {
                        name: `⚔️ ${t.winLoss}`, value: `\`${gamesWon}/${gamesLost}\``, inline: true
                    },
                    {
                        name: `📅 ${t.agentSince}`,
                        value: `<t:${Math.floor(createdAt.getTime() / 1000)}:R>\n└─ ${accountAgeDays} ${lang === 'fr' ? 'jours' : 'days'}`, inline: true
                    },
                    {
                        name: t.combat,
                        value: `\`\`\`yaml\n${t.primarySector}: ${gamingData.game}\n${t.combatMode}: ${gamingData.mode}\n${t.rankTier}: ${gamingData.rank}\`\`\``, inline: false
                    }
                )
                .setFooter({ text: `${guildName} • ${t.digitalSovereignty} • v${version}`, iconURL: guildIcon })
                .setTimestamp();
            ...
            if (target.id === ARCHITECT_ID) {
                dossierEmbed.addFields({ name: t.architectTitle, value: t.architectValue, inline: false });
            }
    ...
    execute: async (interaction, client) => {
        try {
            ...
            // Build a Collection-like mentions.users that has .first() method (Discord.js compatible)
            ...
            const fakeMessage = {
                ...
                content: `/rank ${targetUser.id !== interaction.user.id ? `<@${targetUser.id}>` : ''}`
            };

            const serverSettings = interaction.guild ? client.getServerSettings?.(interaction.guild.id) || {} : { prefix: '.' };

            await module.exports.run(client, fakeMessage, [], db, serverSettings, usedCommand);
        } catch (err) {
