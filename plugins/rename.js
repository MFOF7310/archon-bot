...
module.exports = {
    ...
    run: async (client, message, args, db, serverSettings, usedCommand) => {
        ...
        // ================= GET USER DATA (PER-SERVER) =================
        const guildId = message.guild?.id || 'DM';
        let userData = client.getUserData
            ? client.getUserData(message.author.id, guildId)
            : db.prepare("SELECT username FROM users WHERE id = ? AND guild_id = ?").get(message.author.id, guildId);
        ...
        // ================= UPDATE USER (PER-SERVER) =================
        try {
            if (!userData) {
                // Initialize user if doesn't exist (PER-SERVER: includes guild_id)
                db.prepare(`
                    INSERT INTO users (id, guild_id, username, xp, level, credits, streak_days, last_daily)
                    VALUES (?, ?, ?, 0, 1, 0, 0, 0)
                `).run(message.author.id, guildId, newName);

                if (client.cacheUserData) {
                    client.cacheUserData(message.author.id, guildId, {
                ...
                // Update existing user (PER-SERVER: includes guildId)
                if (client.queueUserUpdate) {
                    client.queueUserUpdate(message.author.id, guildId, {
                        ...userData,
                        username: newName
                    });
                } else {
                    db.prepare("UPDATE users SET username = ? WHERE id = ? AND guild_id = ?").run(newName, message.author.id, guildId);
                }

                // Update cache (PER-SERVER)
                if (client.cacheUserData) {
                    client.cacheUserData(message.author.id, guildId, { ...userData, username: newName });
                }
            }
